...
public class AppMessenger extends AbstractMessengerConnection{
    ...
    void sendMsgTest(Message msg) throws Exception{
        msg.replyTo = mMessenger;//for test only
        sendMessageToServer(msg);
    }
    void sendMsgTest2(Message msg, MultiClientOptions multiClientOptions) throws Exception{
        msg.getData().putString("my_message","hello");
        sendMessageTestToServer(msg,multiClientOptions);
    }
    private void sendMessageTestToServer(Message msg, MultiClientOptions multiClientOptions){
        try {
